...
export const createSpecification = async (req, res) => {
  ...
  const imgPath = path.join(__dirname, "public", "image-view", imgName);
  const compressedImgPath = path.join(
    __dirname,
    "public",
    "image-view",
    `compressed-${imgName}`
  ...
  try {
    ...
    const newSpecification = await prisma.specification.create({
      data: {
        ...
        headTitle5,
        text5,
        imgView: `compressed-${imgName}`,
        urlImgView: `${process.env.APP_HOST}image-view/compressed-${imgName}`,
        trimId: parseInt(trimId, 10),
      },
    });
    ...
    res
      .status(500)
      .json({ msg: "Failed to create Specification.", error: error.message });
  }
};
...
export const updateSpecification = async (req, res) => {
  ...
  try {
    ...
    if (imgView) {
      ...
      const compressedNewImgPath = path.join(
        __dirname,
        "public",
        "image-view",
        `compressed-${imgView.filename}`
      );
      ...
      imgUpdateData.imgView = `compressed-${imgView.filename}`;
      imgUpdateData.urlImgView = `${process.env.APP_HOST}image-view/compressed-${imgView.filename}`;
    ...
    res
      .status(500)
      .json({ msg: "Failed to update specification", error: error.message });
  }
};
...
export const deleteSpecification = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Failed to delete spesification data. " + error.message });
  }
};
